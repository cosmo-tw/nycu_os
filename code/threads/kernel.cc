...
{
	t[threadNum+1] = new Thread(name, threadNum+1);
	t[threadNum+1]->space = new AddrSpace();
	t[threadNum+1]->Fork((VoidFunctionPtr) &ForkExecute, (void *)t[threadNum+1]);
    t[threadNum+1]->setPriority(priority);
	threadNum++;
